...
mean. Further transofmration options are detailed below. For time-series
...
and etc are also available. String parsing methods are available to extract grammatic 
structure shared between cateogric entries. For all cases the functions may generate 
a supplemental column (NArw) with a boolean identifier for cells that were subject to 
infill due to missing or improperly formatted data when the NArw_marker parameter is 
activated.
